<script lang="ts">
    ...
    const apiUrlBase =
        "https://1wwsjsc00f.execute-api.ap-south-1.amazonaws.com/test/company-report-type";
    const adminReportApi =
        "https://1wwsjsc00f.execute-api.ap-south-1.amazonaws.com/test/admin-report-type";
    ...
    async function loadReportSettings() {
        ...
        try {
            ...
            allEmailSettings = await response.json();
            filterReportSettings(); // Filter based on selected device
        } catch (error) {
    ...
    async function saveReportSettings() {
        ...
        try {
            ...
            showAddModal = false;
            await loadReportSettings(); // This will reload and filter settings
        } catch (error) {
            console.error("Failed to save settings:", error);
            if (error instanceof Error) {
                ...
                console.error(
                    "Failed to save settings: An unknown error occurred.",
                );
            }
        } finally {
            isLoading = false;
    ...
    async function updateViewSetting() {
        ...
        try {
            const response = await fetch(
                `${adminReportApi}/update/${company_id}`,
                {
                    method: "PUT",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(setting),
...
<div class="min-h-screen bg-gray-100 p-4 md:p-8">
    ...
    <div class="max-w-5xl mx-auto pt-25">
        ...
        <div
            class="flex flex-row md:flex-row md:items-center items-center justify-between md:justify-between mb-6"
        >
            <h2
                class="text-xl md:text-2xl xl:text-3xl font-bold mb-4 md:mb-0 text-gray-800"
            >
                Report Settings
            </h2>
            <button
        ...
        <div
            class="flex flex-col md:flex-row md:items-center justify-between md:justify-between mb-6"
        >
            <h2
                class="text-xl md:text-2xl xl:text-3xl font-bold mb-4 md:mb-0 text-gray-800"
            >
                ...
                <table class="w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <tr class="text-center">
                            ...
                            >
                                <button
                                    on:click={openViewEditModal}
                                    class="text-[#02066F] hover:text-[#02066F]"
                                >
        ...
        >
            <div class="bg-white rounded-lg w-full max-w-sm mx-4 shadow-xl">
                <div
                    class="flex w-full bg-[#02066F] justify-between p-2 pl-4 pr-4 items-center rounded-t-md text-center"
                >
                    <h3 class="text-2xl font-semibold p-2 text-white">
                        Report Details
                    </h3>
                    <button
                        class="text-gray-400 hover:text-white text-4xl cursor-pointer p-2"
        ...
        >
            <div class="bg-white rounded-lg w-full max-w-sm mx-4 shadow-xl">
                ...
                >
                    ...
                    <button
                        class="text-gray-400 hover:text-white text-4xl cursor-pointer p-2"
                        on:click={() => (showViewEditModal = false)}
                    >
                        ×</button
                    >
                </div>
                ...
                <div class="p-6">
                    <div class="mb-6">
                        <div class="relative">
                            ...
                            >
                                <span
                                    class={viewFrequencies.length === 0
                                        ? "text-gray-500"
                                        : "text-[#02066F] text-base"}
                                >
    ...
    {#if showDeleteModal}
        ...
        >
            <div class="bg-white rounded-lg w-full max-w-md mx-4">
                ...
                >
                    <h3
                        class="text-xl font-bold text-center text-white p-2 justify-center"
                    >
                        Delete
                    </h3>
                    <button
