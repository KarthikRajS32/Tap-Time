<script lang="ts">
  ...
  function filterEmployees() {
    filteredEmployees = employees.filter(emp => emp.IsAdmin === 0);
    admins = employees.filter(emp => emp.IsAdmin === 1);
    superAdmins = employees.filter(emp => emp.IsAdmin === 2);
  ...
  function searchSuperAdmins() {
    ...
    superAdmins = employees.filter(emp =>
      emp.IsAdmin === 2 &&
      (emp.FName.toLowerCase().includes(term) ||
      emp.LName.toLowerCase().includes(term) ||
  ...
  async function submitForm() {
    ...
    try {
      ...
      if (data.error) {
        ...
        successMessage = isEditing ? 'Employee updated successfully' : 'Employee added successfully';
        fetchEmployeeData();
      }
    } catch (error) {
  ...
  async function deleteEmployee() {
    ...
    try {
      loading = true;
      const response = await fetch(`${apiUrlBase}/delete/${currentEmployee.EmpID}/Admin`, {
        method: 'PUT'
      });
  ...
  <div class="max-w-5xl mx-auto">
    <div class="mb-8">
      <div class="flex flex-row sm:flex-row justify-between items-start sm:items-center mb-4 gap-4">
        <h2 class="text-2xl sm:text-3xl font-bold text-gray-800">Employee Details</h2>
        <button
      ...
      <div class="bg-white rounded-xl shadow-md overflow-hidden border border-gray-300">
        <div class="p-4 sm:p-6 overflow-x-auto">
          <div class="flex flex-col sm:flex-row justify-between items-star sm:items-center mb-4 gap-4">
            <div class="flex items-center space-x-2 md:mt-[-16px] text-center justify-center">
            ...
            <div class="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-end gap-2">
              <label class="text-base font-semibold text-gray-800">Search:</label>
              <input
                type="text"
            ...
            <table class="min-w-full divide-y divide-gray-200">
              <thead class="bg-[#02066F] text-white">
                <tr>
                  ...
                  <th class="px-6 py-3 text-center text-base font-bold tracking-wider">
                    Action
                  </th>
                </tr>
              </thead>
              <tbody class="bg-white divide-y divide-gray-200">
                {#each paginatedEmployees as employee (employee.EmpID)}
                  <tr class="hover:bg-gray-50">
                    <td class="px-4 py-4 whitespace-nowrap text-sm font-semibold text-gray-900 text-center">{employee.Pin}</td>
                    <td class="px-4 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">{employee.FName} {employee.LName}</td>
                    <td class="px-4 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">{formatPhoneNumber(employee.PhoneNumber)}</td>
                ...
                {:else}
                  <tr>
                    <td colspan="4" class="px-4 py-4 text-center text-sm text-gray-500">No employees found</td>
                  </tr>
                {/each}
              </tbody>
            </table>
          ...
          <div class="p-4 sm:p-6 overflow-x-auto">
            ...
            <div class="overflow-x-auto">
              <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-[#02066F] text-white">
                  <tr>
                    <th class="px-4 py-3 text-left text-base font-bold tracking-wider">Pin</th>
                    <th class="px-4 py-3 text-left text-base font-bold tracking-wider">Name</th>
                    <th class="px-4 py-3 text-left text-base font-bold tracking-wider">Phone Number</th>
                    <th class="px-4 py-3 text-left text-base font-bold tracking-wider">Action</th>
                  </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                  {#each admins as admin (admin.EmpID)}
                    <tr class="hover:bg-gray-50">
                      <td class="px-4 py-4 whitespace-nowrap text-sm font-semibold text-gray-900 text-center">{admin.Pin}</td>
                      <td class="px-4 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">{admin.FName} {admin.LName}</td>
                      <td class="px-4 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">{formatPhoneNumber(admin.PhoneNumber)}</td>
                      <td class="px-4 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">
                        <div class="flex justify-center space-x-2">
                          <button 
              ...
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  {#each superAdmins as superAdmin (superAdmin.EmpID)}
                    <tr class="hover:bg-gray-50">
                      <td class="px-4 py-4 whitespace-nowrap text-sm font-semibold text-gray-900 text-center">{superAdmin.Pin}</td>
                      <td class="px-4 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">{superAdmin.FName} {superAdmin.LName}</td>
                      <td class="px-4 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">{formatPhoneNumber(superAdmin.PhoneNumber)}</td>
                      <td class="px-4 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">
                        <div class="flex justify-center space-x-2">
                          <button 
                  ...
                  {:else}
                    <tr>
                      <td colspan="4" class="px-4 py-4 text-center text-sm text-gray-500">No super admins found</td>
                    </tr>
                  {/each}
                </tbody>
              </table>
  ...
  {#if showEmployeeModal}
    ...
    >
      <div 
        ...
        transition:fade
      >
        <div class="flex w-full bg-[#02066F] justify-between p-3 pl-4 pr-4 items-center rounded-t-md text-center">
          ...
          <form on:submit|preventDefault={submitForm}>
            ...
            <div class="mb-4">
              ...
              <input
                ...
                class="w-full px-3 py-2 border-2 border-[#02066F] rounded-lg bg-gray-300 font-bold focus:outline-none"disabled
          ...
          <form on:submit|preventDefault={submitForm}>
            ...
            <div class="mb-4">
              ...
              <input
                type="text"
                bind:value={formData.Pin}
                class="w-full px-3 py-2 font-bold border-2 border-[#02066F] rounded-lg bg-gray-200 focus:outline-none "disabled
                placeholder="Pin"
                readonly
              />
            </div>
        ...
        <div class="p-6">
          <form on:submit|preventDefault={submitForm}>
            ...
            <div class="mb-4">
              ...
              />
              {#if errors.PhoneNumber}
                <p class="text-red-500 text-xs italic mt-1">{errors.PhoneNumber}</p>
              {/if}
            </div>
            <div class="mb-4">
              <input
        ...
        <div class="py-6 px-0">
          {#if currentEmployee}
            <p class="text-center text-gray-800 font-bold text-lg mb-6">
              Are you sure, you want to remove the {currentEmployee.IsAdmin === 0 ? 'employee' : currentEmployee.IsAdmin === 1 ? 'admin' : 'super admin'}?
            </p>
          {:else}
